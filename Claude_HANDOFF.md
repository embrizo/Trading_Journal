...
- **4H watch dropped** (2026-09-24) on the strength of the M6 numbers — the local `config.yaml` now starts a single `SOL-USDT-SWAP 1D` watcher (the 4H entry is commented out with the reason, not deleted). `config.example.yaml` still lists 4H because it demonstrates the multi-watch format, but now says plainly that it is not a recommendation. Dropping a watch only stops alerts; the dashboard still charts any timeframe, and the 4H signals already in the journal are untouched.
