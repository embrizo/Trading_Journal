...
- **Timeframes:** **1D only** since 2026-09-24 — in `config.example.yaml` as well as the working configs. 4H was watched until then; M6 measured it as break-even before costs on SOL/BTC/ETH (PF 1.02–1.14 vs 1D's 1.66–2.18, see `BACKTEST_REPORT.md`) and the user dropped it. The engine still supports any bar the exchange does, and 4H trades can still be journaled by hand; this is about which timeframes raise alerts.
...
```bash
# All tests — numpy + pytest (+ aiohttp/websockets/mcp/anthropic for the wiring tests); 317 pass, 3 live skipped
...
0. **Journal + AI coach — all code done (J0–J6, 2026-09-20); Gemini backend done (commit `24aa7a5`).** **Live checks still owed:** (a) `.mcp.json` loads in a fresh Claude Code session (confirmed 2026-09-20); (b) ~~a real LLM call~~ **`/ask` done 2026-09-23**; **`/review` and the weekly narrative done 2026-09-24** on Gemini, against a scratch copy of the journal (one trade closed there so there was something to review). Only the Anthropic path (`ai.provider: anthropic`) is still untested live; (c) ~~Telegram bot~~ **mostly done 2026-09-23** — `/help` and `/trade` verified live over a real bot token; send `/stats` once to close it out; (d) OKX from a machine where it isn't blocked (403/DNS-blocked on two different machines/ISPs so far — `exchange: binance` is the working fallback); (e) Pi: `docker compose up -d --build`, open `http://<pi>:8787/`.
...
- **`/review` and the weekly narrative verified live on Gemini** (scratch copy of the journal, one trade closed in it). `/review` produced correct FACTS/METRICS/RULE VIOLATIONS/OBSERVATIONS/QUESTIONS and spotted that the initial stop sat 0.001 from the liquidation price — and `parity_check` correctly flagged that 0.001 as a number no tool returned (the model subtracted it). Two output defects found and fixed in `prompts.py`: Gemini emitted **LaTeX** (`$n = 1$`) and full float precision (`0.8920327624720762 R`); all three prompts now say plain text only, and that quoting a long decimal to 2 dp is presentation rather than calculation. Re-ran: clean.
- **Bug: `parity_check` crashed on a non-finite metric.** `round(inf)` / `int(inf)` raise, and `profit_factor` is `inf` whenever a period has no losses — so the weekly narrative blew up with `OverflowError` exactly when a trader was having a good week, and it would have taken the *scheduled* Monday report down with it. `_numbers_in_results` and `_norm` now skip non-finite values (there is no digit spelling a reply could quote anyway). This is the same `inf` family as the `/api/summary` JSON bug and the PF-sign bug — **when touching metrics, always test the no-losses case.**
